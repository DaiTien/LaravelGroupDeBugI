...
@section('titleForm')
    <div class="Page breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{__('TitleUpdateShowTime')}}</li>
        </ol>
    </div>
@endsection
@section('content')
            ...
            <div class=row>
                <div class="col-8 mx-auto">
                        ...
                        <div class="col-sm-9">
                            <select name="movie" class="form-control">
                                ...
                                @foreach($movie as $movie)
                                    <option value="{{$movie->id}}" {{ (old('movie',$data->movie_id) == $movie->id ? "selected":"") }}>{{$movie->name}}</option>
                                @endforeach
                                @error('movie')
                        ...
                        <div class="col-sm-9">
                            <select name="room" class="form-control">
                                ...
                                @foreach($room as $room)
                                    <option value="{{$room->id}}" {{ (old('room',$data->room_id) == $room->id ? "selected":"") }}>{{$room->name}}</option>
                                @endforeach
                                @error('room')
                                <span class="text-danger font-weight-bold">{{ $message }}</span>
                                @enderror
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="date" placeholder="show date" name="show_date" class="form-control" value="{{old('show_date',$data->show_date)}}" min="date()"/>
                            @error('show_date')
                            <span class="text-danger font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="time" placeholder="time start" name="time_start" class="form-control" value="{{old('time_start',$data->time_start)}}"/>
                            @error('time_start')
                            <span class="text-danger font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="time" placeholder="time end" name="time_end" class="form-control" value="{{old('time_end',$data->time_end)}}"/>
                            @error('time_end')
                            <span class="text-danger font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>
